...
class singlesilder(QWidget):  # 单控制条的窗口
    ...
    def slider1change(self):
        main["ImgTodoListWindow"].SetCommand(self.num, True, "%s:%d,%d" % (
        self.windowTitle(), self.slider1.value(), self.slider2.value()))
...
class threesilder(QWidget):  # 三控制条的窗口
    ...
    def slider1change(self):
        main["ImgTodoListWindow"].SetCommand(self.num, True, "%s:%d,%d,%d" % (
        self.windowTitle(), self.slider1.value(), self.slider2.value(), self.slider3.value()))
        main["OptimizeWindow"].MakeImg()
        if self.label1datanum > 1:
            if len(self.label1data) > self.slider1.value():
                self.label1.setText(self.label1data[self.slider1.value()])
